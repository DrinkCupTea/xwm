...
Drw* drw_create(Display* dpy, int screen, Window root, unsigned int w, unsigned int h) {
    Drw* drw = reinterpret_cast<Drw*>(ecalloc(1, sizeof(Drw)));
...
static Fnt* xfont_create(Drw* drw, const char* fontname, FcPattern* fontpattern) {
    ...
    font = reinterpret_cast<Fnt*>(ecalloc(1, sizeof(Fnt)));
    font->xfont = xfont;
...
Clr* drw_scm_create(Drw* drw, const char* clrnames[], size_t clrcount) {
    ...
    if (!drw || !clrnames || clrcount < 2 || !(ret = reinterpret_cast<Clr*>(ecalloc(clrcount, sizeof(XftColor)))))
        return NULL;
...
Cur* drw_cur_create(Drw* drw, int shape) {
    ...
    if (!drw || !(cur = reinterpret_cast<Cur*>(ecalloc(1, sizeof(Cur)))))
        return NULL;
