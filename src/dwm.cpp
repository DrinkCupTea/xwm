...
void applyrules(Client* c) {
    ...
    for (i = 0; i < LENGTH(rules); i++) {
        r = &rules[i];
        if ((!r->title || strstr(c->name, r->title)) && (!r->_class || strstr(_class, r->_class)) &&
...
Monitor* createmon(void) {
    ...
    m = reinterpret_cast<Monitor*>(ecalloc(1, sizeof(Monitor)));
        ...
        for (i = 0; i < LENGTH(buttons); i++)
            if (buttons[i].click == ClkClientWin)
                for (j = 0; j < LENGTH(modifiers); j++)
                    XGrabButton(dpy, buttons[i].button, buttons[i].mask | modifiers[j], c->win, False, BUTTONMASK, GrabModeAsync, GrabModeSync, None,
                                None);
    }
}
...
void manage(Window w, XWindowAttributes* wa) {
    ...
    c = reinterpret_cast<Client*>(ecalloc(1, sizeof(Client)));
    c->win = w;
    /* geometry */
    c->x = c->oldx = wa->x;
    c->y = c->oldy = wa->y;
...
void setup(void) {
    ...
    /* init appearance */
    scheme = reinterpret_cast<Clr**>(ecalloc(LENGTH(colors), sizeof(Clr*)));
    for (i = 0; i < LENGTH(colors); i++)
